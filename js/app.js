...
// ==============================
// LINKS DE FINANCIAMIENTO
// ==============================
const PDF_FINANCIAMIENTO = {
  oferta: "",
  descuentos: "",
};
...
function abrirPdfFinanciamiento(tipo) {
  ...
  if (!url || url.trim() === "" || url.includes("REEMPLAZA")) {
    mostrarToast("Link de " + tipo + " aún no configurado. Usa el panel Admin.", "info");
    return;
  }
...
window.onload = async () => {
  ...
  try {
    ...
    mostrarToast("Error cargando catálogo de vehículos", "error");
    return;
  }
...
function actualizarPrecio() {
  ...
  if (fichaActual) {
    ...
    // Verificar si el modelo tiene definida una ficha para este tipo (gasolina o hev)
    ...
    if (tieneFicha) {
      ...
      // Si el link está vacío, indicarlo visualmente pero no ocultar el botón
...
function abrirFichaTecnica() {
  ...
  if (!url || url.trim() === "") {
    mostrarToast("Configura el link en el panel Admin → Catálogo", "info");
    return;
  }
  ...
  if (telefono) {
    ...
    const msg = encodeURIComponent(
      `${nombre ? "Hola *" + nombre + "*, a" : "A"}quí te comparto la ficha técnica del *${modeloSeleccionado} ${versionSeleccionada}*:\n\n${url}`,
    );
...
function openWhatsApp(numeroWa, mensaje, delayMs = 0) {
  const fn = () => {
    const w = window.open(`https://wa.me/${numeroWa}?text=${mensaje}`, "_blank");
    if (!w) location.href = `https://wa.me/${numeroWa}?text=${mensaje}`;
  };
...
function enviarACliente() {
  ...
  // Link de ficha técnica si está configurado
  const btnFicha = document.getElementById("btn-ficha");
  const linkFicha = (btnFicha && btnFicha.style.display !== "none" && btnFicha.dataset.url)
    ? btnFicha.dataset.url.trim() : "";

  // Emojis como caracteres UTF-8 directos — WhatsApp los renderiza correctamente
  const saludo = nombre
    ? ["👋", " Hola *", nombre, "*, soy *Israel Ortiz*, asesor de ventas en *Honda Montejo*."].join("")
    : "👋 Hola, soy *Israel Ortiz*, asesor de ventas en *Honda Montejo*.";

  let lineas = [
    saludo,
    "",
    "🚗 Te comparto la información del vehículo de tu interés:",
  ...
  // Codificar preservando emojis — encodeURIComponent los rompe en WhatsApp Android
  const textoCodificado = texto.split("").map(function(c) {
    const code = c.codePointAt(0);
    // Emojis y caracteres especiales >U+00FF: dejar sin codificar (WhatsApp los lee bien)
    if (code > 0x00FF) return c;
    // Caracteres especiales ASCII que deben codificarse
...
function saveLead(lead) {
  ...
  setLeads(leads);
  updateLeadCounter();
  // Fire-and-forget a Firestore, no bloquea UI
  db.collection("leads")
    .doc(lead.id)
    .set(lead)
    .catch(() => {});
...
function renderLeadsList() {
  ...
  if (!pageItems.length) {
    ...
    cont.innerHTML = `<div class="lead-cards-list">${pageItems
      .map((l) => {
        const ultimaNota = l.notas && l.notas.length ? `<p class="lead-card-nota">${escapeHtml(l.notas[l.notas.length - 1].texto)}</p>` : "";
        return `
        <div class="lead-card" onclick="abrirDetalleLead('${l.id}')">
          <div class="lead-card-top">
            <div><span class="lead-card-name">${escapeHtml(l.name || "Sin nombre")}</span>${statusBadge(l.status || "nuevo")}</div>
            <span class="lead-card-date">${formatDateTime(l.dateISO)}</span>
          </div>
          <div class="lead-card-mid">
            <span>🚗 ${escapeHtml(l.model || "")} ${escapeHtml(l.version || "")}</span>
          ...
          <div class="lead-card-actions" onclick="event.stopPropagation()">
            <input type="checkbox" class="lead-check" data-id="${l.id}" onchange="updateLeadSelectionUI()">
          </div>
        </div>`;
      })
      .join("")}</div>`;
  ...
  log.innerHTML = lead.notas
    ...
    <div class="nota-item">
      <div class="nota-header">
        <span class="nota-fecha">${formatDateTime(n.fecha)}</span>
      ...
      <p class="nota-texto">${escapeHtml(n.texto)}</p>
    </div>`,
    )
    .reverse()
    .join("");
...
function agregarNotaLead() {
  ...
  // Limpiar input y actualizar UI en el mismo tick — sin esperar Firestore
  if (inputNota) inputNota.value = "";
  renderNotasLog(leads[idx]);

  // Firestore y renderLeadsList en background — no bloquean el click
  setTimeout(() => {
    sincronizarLeadUpdateFirestore(leads[idx]);
    renderLeadsList();
  }, 0);
...
function abrirModalGuardia(fechaStr, modoEdicion = false, eventoData = null) {
  ...
  // requestAnimationFrame en lugar de setTimeout para no bloquear el hilo
...
// ==============================
// MODAL GUARDIA — listeners
// (usando DOMContentLoaded para evitar errores si el DOM no está listo)
// ==============================
document.addEventListener("DOMContentLoaded", () => {
  // --- Cerrar modales ---
  document.getElementById("btnCerrarModal").onclick = cerrarModalGuardia;
  document.getElementById("btnCancelar").onclick = cerrarModalGuardia;
  document.getElementById("modalGuardia").addEventListener("mousedown", (e) => {
    if (e.target === e.currentTarget) cerrarModalGuardia();
  });

  document.getElementById("btnCerrarModalLead").onclick = cerrarModalLead;
  document.getElementById("modalLead").addEventListener("mousedown", (e) => {
    if (e.target === e.currentTarget) cerrarModalLead();
  });

  document.getElementById("btnCerrarEditarNota").onclick = cerrarModalEditarNota;
  document.getElementById("modalEditarNota").addEventListener("mousedown", (e) => {
    if (e.target === e.currentTarget) cerrarModalEditarNota();
  });

  const btnX = document.getElementById("btnCerrarRolMes");
  const mRol = document.getElementById("modalRolMes");
  if (btnX) btnX.onclick = cerrarModalRolMes;
  if (mRol)
    mRol.addEventListener("mousedown", (e) => {
  ...
  // --- GUARDAR GUARDIA
  // Cierra el modal de inmediato y escribe a Firestore en background (elimina Violation click)
  document.getElementById("guardarGuardia").addEventListener("click", () => {
    ...
    // Cerrar modal ANTES de la escritura → elimina el Violation
    cerrarModalGuardia();
  ...
  document.getElementById("btnEliminarGuardia").addEventListener("click", () => {
    ...
    // Cerrar modal ANTES → elimina el Violation
    cerrarModalGuardia();
  ...
  // --- TIPO EVENTO (guardia / recordatorio)
...
function renderRolGrid() {
  ...
  for (let d = 1; d <= dias; d++) {
    ...
    html += `<div class="rol-day ${clase}" data-fecha="${fecha}" onclick="toggleRolDia('${fecha}')">
      <span class="rol-day-num">${d}</span><span class="rol-day-turno">${label}</span>
    </div>`;
  }
  html += `</div>`;
  grid.innerHTML = html;
...
// Función síncrona — libera el hilo del click inmediatamente
function guardarRolMes() {
  ...
  // Lanzar el trabajo pesado fuera del handler del click
